...
# pickle):
try:
    if os.path.isfile( "%s.tmp" % options.scheduler_stamps_file ) and not os.path.isfile(options.scheduler_stamps_file):
        os.rename(  "%s.tmp" % options.scheduler_stamps_file,  options.scheduler_stamps_file )
...
def write_scheduler_stamps():
    ...
    try:
        try:
            ...
            log(" write_scheduler_stamps NameError: line %d: %s" % ( sys.exc_info()[2].tb_lineno, e))
    except IOError as e:
        log(" write_scheduler_stamps IOError: line %d: %s" % ( sys.exc_info()[2].tb_lineno, e))
...
def cron_backup(title):
    ...
    if options.verbose:
        ...
        print("[%s] stamp=%f (%s)" % (title, stamp, time.strftime("%x %X %Z", time.localtime(stamp))))
        STAMPS_dict = dict(map(lambda item: ( item[0], time.strftime("%x %X %Z", time.localtime(item[1]) ) ) , STAMPS.iteritems()))
        print("[%s] STAMP=%f (%s)" % (title, STAMP, STAMPS_dict.get(title,0)))
...
def schedule_jobs(config):
    ...
    for title in config:
        ...
        for cron_times in cron_scheduler:
            if str(cron_times[0]) == '':
                sched.add_cron_job( cron_STAMP, args=[title], minute=str(cron_times[1]), coalesce=True )
            elif str(cron_times[1]) == '':
                sched.add_cron_job( cron_STAMP, args=[title], hour=str(cron_times[0]), coalesce=True )
            else:
                sched.add_cron_job( cron_STAMP, args=[title], hour=str(cron_times[0]), minute=str(cron_times[1]), coalesce=True )

        if cron_scheduler != []:
            if options.verbose:
                print("ADDING [%s]" % title)
            sched.add_interval_job( cron_backup, args=[ title ], minutes=1 )
...
def listen(event):
    if event.exception:
        ...
        log_msg = ("".join(traceback.format_tb(event.traceback))).splitlines(True)
        log_msg = [">>> %s" % line for line in log_msg]
        log("\n%s" % "".join(log_msg))
# I think the following line gives: Error function takes exactly 1 argument (0 given):
sched.add_listener( listen, EVENT_JOB_ERROR )

def log_STAMPS():
    STAMPS_list = map(lambda item: ( item[0], time.strftime("%x %X %Z", time.localtime(item[1]) ) ) , STAMPS.iteritems() )
...
for job in sched.get_jobs():
    print("%s(%s) (trigger %s, next run at %s)" % (job.name, job.args, job.trigger, job.next_run_time))
